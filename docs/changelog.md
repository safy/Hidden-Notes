...
- **Task List - список задач с чекбоксами**: Возможность создавать списки задач с чекбоксами для отметки выполнения
  ...
  - **Улучшенная верстка** (обновлено 2025-10-17):
    - Чекбокс и текст расположены рядом (`gap: 0.5rem` в label)
    - Текст выполненных задач зачеркнут и полупрозрачный (`text-decoration: line-through` + `opacity: 0.6`)
    - Применяется к всем дочерним элементам текста (`> div > *`)
    - Чекбокс не сжимается при длинном тексте (`flex-shrink: 0`)
    - Cursor pointer на label для лучшего UX
    - Чекбокс выровнен с первой строкой текста (`margin-top: 0.2em`)
