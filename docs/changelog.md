...
### Изменено
- **LinkBubbleMenu - улучшена логика работы**: 
  - **Создание новой ссылки**: Выделить текст → Нажать кнопку ссылки → Появляется BubbleMenu с input полем
  - **Редактирование существующей ссылки**: Кликнуть на ссылку → Появляется BubbleMenu с кнопками (открыть, редактировать, удалить)
  - Убран prompt диалог, вся работа через BubbleMenu UI
  - Более предсказуемое и интуитивное поведение как в примере Tiptap
  - Добавлена связь между EditorArea, TiptapEditor и LinkBubbleMenu через состояние isCreatingLink
