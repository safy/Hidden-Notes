...
- **Task List - список задач с чекбоксами**: Возможность создавать списки задач с чекбоксами для отметки выполнения
  ...
  - **Улучшенная верстка**:
    - Чекбокс и текст выровнены по центру (`align-items: center`)
    - Текст выполненных задач перечеркнут (`text-decoration: line-through`)
    - Выполненные задачи визуально приглушены (opacity: 0.7, muted color)
    - Курсор pointer на чекбоксах
