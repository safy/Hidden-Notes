...
## [2025-11-02] - feat(editor): улучшенные курсоры для изменения ширины колонок

### Добавлено
- **Интерактивные resize handles** для колонок таблицы:
  - Курсор `col-resize` (↔️ стрелочки) при наведении на границу колонки
  - Увеличена ширина handle до 8px для лучшей интерактивности
  - Визуальный индикатор: **яркий синий цвет (#3b82f6)** при hover
  - Плавная анимация `transition: background-color 0.2s ease`
- **Класс `.resize-cursor`** для всего редактора во время resize операции

### Изменено
- **Курсоры ячеек**:
  - По умолчанию `cursor: text` для td/th (можно вводить текст)
  - При наведении на resize handle → `cursor: col-resize` (приоритет)
- **CSS стили** в `globals.css`:
  - Удален `pointer-events: none` с resize handle
  - Добавлен z-index для корректного отображения
- **TableTriggerButton**:
  - Иконка таблицы теперь `text-foreground` (темная, заметная)
  - Сетка выбора размера: белые ячейки с серыми границами (border-2)
  - Выбранные ячейки: яркий синий фон (#3b82f6)
- **Кастомный курсор col-resize**:
  - Вместо стандартного белого курсора используется **SVG с черными стрелочками**
  - Хорошо видно на любом фоне (светлом и темном)
- **Оптимизация resize handle**:
  - Уменьшена ширина с 8px до 6px (активная область клика)
  - Визуальная линия всего 2px (при hover)
  - Handle растягивается на всю высоту таблицы (height: 100vh с overflow: visible)
  - Псевдоэлемент ::after для тонкой синей линии при наведении

### UX улучшения
- ✅ Понятно где можно изменить ширину колонки
- ✅ Визуальная обратная связь при наведении (яркая синяя полоса)
- ✅ Консистентные курсоры (text для текста, col-resize для границ)
- ✅ Иконка таблицы и сетка выбора хорошо видны на любом фоне

---

## [2025-11-02] - refactor(editor): упрощение UI таблиц - удаление дублирующего меню

### Удалено
- **TableCellMenuHandler** и связанные компоненты:
  - TableCellButton - круглая кнопка на ячейке
  - TableCellOptionsMenu - всплывающее меню
  - Визуальное выделение ячеек (фиолетовая рамка)
  - ProseMirror Decoration API для класса `selectedCell`
- **CSS стили**:
  - `.selectedCell` outline стили
  - `position: relative` для ячеек таблицы
  - Комментарии о call point кнопке

### Изменено
- **TableCellEnhanced.ts**:
  - Удален `addProseMirrorPlugins()` с Decoration API
  - Оставлены только кастомные атрибуты: `backgroundColor`, `align`
  - Упрощенная архитектура без визуального выделения
- **TableHeaderEnhanced.ts**:
  - Заменен атрибут `verticalAlign` на `align` (консистентность с TableCell)
- **TiptapEditor.tsx**:
  - Удален импорт и использование `TableCellMenuHandler`

### Причина
Функционал дублировался между **TableCellOptionsMenu** (меню у ячейки) и **TableToolbar** (панель инструментов). Для упрощения UI и устранения путаницы оставлен только TableToolbar - единое место для всех операций с таблицами.

### Результат
- 📦 **Bundle size**: -6.79 kB (764.71 kB → 757.92 kB)
- 🎯 **Чистый UI**: один источник правды для операций с таблицами
- ⚡ **Производительность**: меньше React компонентов, нет Decoration API
- 🧹 **Код**: удалено ~300 строк неиспользуемого кода

---

## [2025-11-02] - feat(editor): адаптивное переключение Toolbar для таблиц

### Добавлено
- **TableToolbar** - компактная панель инструментов для работы с таблицами
  - Управление строками: добавить выше/ниже, удалить
  - Управление колонками: добавить слева/справа, удалить
  - Настройки ячеек: цвет фона, выравнивание, очистка
  - Удаление таблицы
- **Автоматическое переключение Toolbar** в EditorArea:
  - При перемещении курсора в таблицу → показывается TableToolbar
  - При выходе из таблицы → возвращается стандартный Toolbar
  - Отслеживание через `editor.on('transaction')` и `editor.on('selectionUpdate')`

### Изменено
- **EditorArea.tsx**:
  - Добавлено состояние `isInTable` для отслеживания позиции курсора
  - Условный рендер: `{isInTable ? <TableToolbar /> : <Toolbar />}`
  - useEffect с подпиской на события транзакций редактора

### Технические детали
- **Отслеживание позиции курсора**:
  - Проверка родительских узлов `$from.node(i)` на `node.type.name === 'table'`
  - События: `transaction`, `selectionUpdate` для реактивного обновления
- **Компактный UI**:
  - Все инструменты таблицы в одной строке (экономия места для узкой панели)
  - Dropdown меню для цвета и выравнивания
  - Иконки Lucide: ArrowUp/Down/Left/Right, Rows, Columns, Palette, Trash2
- **Интеграция с i18n**:
  - Все tooltips переведены через `useTranslation()`
  - Ключи в `ru.json`: `table.addRowBefore`, `table.cellColor`, etc.

### Преимущества
- 🎯 **Контекстно-зависимый UI** - показываются только нужные инструменты
- 💾 **Экономия места** - критично для узкой Side Panel
- ⚡ **Быстрый доступ** - все операции с таблицей в одном месте
- 🎨 **Консистентный дизайн** - shadcn/ui компоненты

---

## [2025-11-02] - feat(editor): интерактивное меню для ячеек таблиц (ФИНАЛЬНАЯ версия)

### Добавлено
- **Фиолетовая рамка** (2px) вокруг активной ячейки при клике
- **Круглая кнопка** (call point) НА рамке справа:
  - Изначально: маленькая точка 12px БЕЗ белых точек
  - При hover: увеличивается до 20px + появляются 4 белые точки (2x2)
  - Позиция: `right: -6px` (12px) / `right: -10px` (20px) - половина снаружи рамки
- **Меню настроек** при клике на кнопку:
  - **Цвет**: 7 цветов фона + по умолчанию
  - **Выравнивание**: слева/центр/справа
  - **Очистить**: удаление содержимого
- Расширенные TableCell и TableHeader:
  - `backgroundColor` - цвет фона
  - `verticalAlign` - вертикальное выравнивание

### Технические детали
- **TableCellEnhanced.ts** - с **ProseMirror Decoration API**:
  - Класс `.selectedCell` добавляется через `Decoration.node()`
  - НЕ стирается при перерисовке DOM (в отличие от `classList.add()`)
  - Plugin отслеживает позицию курсора в таблице
- **TableHeaderEnhanced.ts** - поддержка атрибутов backgroundColor, verticalAlign
- **TableCellOptionsMenu.tsx** - меню Color, Alignment, Clear
- **TableCellMenuHandler.tsx** - обработчик кликов:
  - Вычисляет расстояние от клика до позиции кнопки
  - Открывает меню при клике в радиус 12px от центра кнопки
- **CSS изменения**:
  - `border-collapse: separate` вместо `collapse` (иначе ::after невидим!)
  - Маленькая точка 12px → увеличение до 20px при hover
  - 4 белые точки появляются только при hover
  - Плавная анимация через `transition: all 0.2s ease`

### Решённые проблемы
1. ❌ React NodeView → ✅ Decoration API (нет конфликтов)
2. ❌ classList.add() → ✅ Decoration.node() (не стирается)
3. ❌ border-collapse: collapse → ✅ separate (::after виден)
4. ❌ background-color + background-image → ✅ единый background
5. ❌ Кнопка при hover → ✅ Кнопка при клике на ячейку

### Визуальное поведение
- **Клик на ячейку**: фиолетовая рамка + маленькая точка 12px
- **Hover на точку**: увеличение до 20px + 4 белые точки
- **Клик на точку**: меню с настройками

### Совместимость
- ✅ БЕЗ подписки Tiptap Start plan
- ✅ Не ломается и не зависает
- ✅ 0 ошибок линтера
- ✅ Bundle: 1,045 KB

---

## [2025-11-02] - feat(editor): интерактивное меню для работы с ячейками таблиц

### Добавлено
- Визуальное выделение активной ячейки таблицы фиолетовой рамкой (как в Tiptap Table Node)
- Круглая кнопка (call point) для вызова меню настроек ячейки
- Меню настроек ячейки с опциями:
  - **Color**: выбор цвета фона ячейки (7 предустановленных цветов)
  - **Alignment**: выравнивание текста (left, center, right)
  - **Clear contents**: очистка содержимого ячейки
- Поддержка атрибута `backgroundColor` для ячеек таблиц
- Поддержка атрибута `verticalAlign` для вертикального выравнивания

### Технические детали
- Создан `src/components/TiptapEditor/extensions/TableCellWithMenu.ts`:
  - Расширяет стандартный `TableCell` от Tiptap
  - Добавляет атрибуты `backgroundColor` и `verticalAlign`
  - ProseMirror Plugin для добавления call point кнопок в DOM
- Создан `src/components/TiptapEditor/TableCellMenu.tsx`:
  - Popover меню с настройками ячейки
  - Подменю для выбора цвета фона (7 цветов + default)
  - Подменю для выравнивания текста
  - Кнопка очистки содержимого
- Создан `src/components/TiptapEditor/TableCellMenuManager.tsx`:
  - Управляет состоянием меню и обработкой кликов
  - Автоматически открывает меню при клике на call point
  - Закрывает меню при клике вне его
- Обновлен `src/components/TiptapEditor/TiptapEditor.tsx`:
  - Заменен стандартный `TableCell` на `TableCellWithMenu`
  - Добавлены компоненты `TableCellMenuManager` и `TableFloatingMenu`
- Добавлены CSS стили в `src/styles/globals.css`:
  - Фиолетовая рамка для активной ячейки (`outline: 2px solid rgb(147, 51, 234)`)
  - Стили для call point кнопки (круглая, фиолетовая, с иконкой)
  - Hover эффекты для call point
  - Поддержка background color через CSS переменные
- Линтер: 0 ошибок

### Вдохновение
Реализация основана на документации Tiptap Table Node (https://tiptap.dev/docs/ui-components/node-components/table-node):
- Визуальная обратная связь при выборе ячейки
- Call point для быстрого доступа к настройкам
- Структурированное меню с подразделами

### Совместимость
- ✅ Все существующие таблицы продолжают работать
- ✅ TableFloatingMenu для управления строками/столбцами сохранен
- ✅ Базовая функциональность таблиц не затронута
...
## [2025-10-23] - 🎯 Полная система drag & drop для папок
...
#### 🐛 Исправлена логика обводки для перетаскивания папок
...
#### 🐛 Исправлена логика вложения папок
...
#### 🐛 Финальное исправление: Правильная логика различения между сортировкой и вложением
...
**Иконки папок** (16 предустановленных):
📁 💼 🏠 📚 💻 🎯 📊 🎨 🔬 🎓 💡 📝 🎵 🎮 ✈️ 🍔
