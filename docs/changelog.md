...
### Добавлено
- 🎯 Новое расширение DraggableBlock для Tiptap
- 📦 Функционал перетаскивания блоков как в Notion:
  ...
  - Визуальный feedback (opacity, border, пульсирующий эффект)
  - Автоматическое сохранение позиции блока
- 🎨 CSS стили для drag & drop:
  - Hover эффект с появлением drag handle
  - Active состояние при перетаскивании
  - Drop target индикатор с animation
  - Cursor feedback (grab/grabbing)
- 📚 Документация:
  - `DRAG_DROP_DEBUG.md` - полное руководство отладки с примерами Chrome DevTools
  - `DRAG_DROP_QUICKSTART.md` - быстрый старт за 2 минуты
  - Console тесты для проверки функционала

### Изменено
- 🔄 Обновлен TiptapEditor.tsx для интеграции DraggableBlock
- 🎨 Расширен globals.css с новыми стилями для drag & drop

### Технические детали
- Использован ProseMirror Plugin с handleDOMEvents
- Implement dragstart, dragover, dragleave, drop, dragend events
- Транзакции для безопасного перемещения блоков
- Storage и сохранение через Chrome Storage API (автоматически)
