...
### Изменено
- **LinkBubbleMenu - финальная версия с полным UI**: 
  - **Создание новой ссылки**: Выделить текст → Нажать кнопку ссылки в toolbar → Появляется BubbleMenu с:
    ...
    - Кнопка "Открыть в новой вкладке" (ExternalLink)
    - Кнопка "Удалить ссылку" (Link2Off)
  ...
  - `shouldShow` теперь не требует выделения текста при `isCreatingLink=true`
