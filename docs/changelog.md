...
- **Task List - список задач с чекбоксами**: Возможность создавать списки задач с чекбоксами для отметки выполнения
  ...
  - **Улучшенная верстка**:
    - Чекбокс выровнен по верхней линии текста (`align-items: flex-start` + `margin-top: 0.15em`)
    - Текст выполненных задач зачеркнут и светлее (`text-decoration: line-through` + `muted-foreground`)
    - Чекбокс не сжимается при длинном тексте (`flex-shrink: 0`)
    - Курсор pointer на чекбоксах для лучшего UX
...
### Изменено
- **LinkBubbleMenu - адаптивная финальная версия**: 
  - **Создание новой ссылки**: Выделить текст → Нажать кнопку ссылки в toolbar → Появляется BubbleMenu с:
    - Input поле "Вставьте ссылку..." (адаптивная ширина)
    - Кнопка "Сохранить" (Check icon)
    - Кнопка "Открыть в новой вкладке" (ExternalLink icon)
    - Кнопка "Удалить ссылку" (Link2Off icon)
  - **Редактирование существующей ссылки**: Кликнуть на ссылку → Появляется BubbleMenu с просмотром URL и кнопками
  - **Адаптивный дизайн**:
    - Максимальная ширина: `min(90vw, 500px)` - подстраивается под размер экрана
    - Input с гибкой шириной: `min-w-[180px] max-w-[300px]` с `flex-1`
    - Кнопки сгруппированы с `flex-shrink-0` для предотвращения сжатия
    - Сепараторы скрыты на маленьких экранах (`hidden sm:block`)
    - `flex-wrap` позволяет элементам переноситься на новую строку при необходимости
  - Использован `useRef` для хранения актуального значения `isCreatingLink` (решение проблемы с `useCallback`)
  - Автоматическая установка focus на редактор при активации режима создания ссылки
  - Полное соответствие UI из примера Tiptap
  - Escape закрывает меню создания ссылки
