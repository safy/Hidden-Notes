...
### Изменено
- **LinkBubbleMenu - изменена логика появления**: 
  - BubbleMenu теперь появляется **только** при клике на существующую ссылку (для редактирования)
  - Больше НЕ появляется автоматически при выделении текста
  - Создание новой ссылки теперь через кнопку в toolbar с prompt диалогом
  - Более предсказуемое и понятное поведение UI
