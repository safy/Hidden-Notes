...
## [2025-10-23] - 🔧 Исправление drag & drop для папок
...
- **Drag & Drop заметок в папки**: Восстановлена функциональность перетаскивания заметок в папки
  ...
  - Импортирован `useDroppable` из `@dnd-kit/core`

### Технические детали
- Проблема возникла из-за временного отключения drag & drop для отладки кликов
- Функциональность кликов по папкам сохранена полностью
- ID папки теперь соответствует ожидаемому формату в `Sidebar.tsx`
