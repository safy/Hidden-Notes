...
## 📅 [2025-11-02] | Адаптивное переключение Toolbar для таблиц

### 📊 Задача
Реализовать **контекстно-зависимый UI** для работы с таблицами:
- При перемещении курсора в таблицу показывать **TableToolbar** с инструментами для таблиц
- При выходе из таблицы возвращать стандартный **Toolbar**
- Экономия места в узкой Side Panel (~400px)

### 🎯 Мотивация
**Проблема**: Side Panel имеет ограниченную ширину, и одновременное отображение основного Toolbar + TableFloatingMenu + TableCellOptionsMenu создает визуальную перегрузку. Пользователь теряется среди множества панелей инструментов.

**Решение**: Показывать только **релевантные инструменты** в зависимости от контекста. Когда курсор в таблице - нужны только инструменты для таблиц. Когда курсор вне таблицы - нужен основной Toolbar.

### ✅ Решения

#### 1. Создан TableToolbar компонент
**Что сделано**:
- Компактная панель инструментов, объединяющая:
  - Управление строками (добавить выше/ниже, удалить)
  - Управление колонками (добавить слева/справа, удалить)
  - Настройки ячеек (цвет, выравнивание, очистка)
  - Удаление всей таблицы
- Использованы иконки Lucide: `ArrowUp`, `ArrowDown`, `ArrowLeft`, `ArrowRight`, `Rows`, `Columns`, `Palette`, `Trash2`
- Dropdown меню для цвета (7 вариантов + default) и выравнивания (left/center/right)

**Код**:
```typescript
// src/components/TiptapEditor/TableToolbar.tsx
...
**Преимущества**:
- ✅ Все инструменты для таблицы в одном месте
- ✅ Компактный UI (одна строка кнопок)
- ✅ Интеграция с i18n
- ✅ Логика запрета удаления последней строки/колонки

#### 2. Отслеживание позиции курсора
**Проблема**: Как определить, находится ли курсор в таблице?

**Решение**:
- В `EditorArea.tsx` добавлено состояние `isInTable`
- useEffect с подпиской на события редактора:
  - `editor.on('transaction')` - срабатывает при любом изменении
  - `editor.on('selectionUpdate')` - срабатывает при перемещении курсора
- Функция `checkIfInTable()` проверяет родительские узлы:
...
const checkIfInTable = () => {
  ...
  // Проходим по родительским узлам
...
**Почему это работает**:
- `$from.depth` - глубина вложенности текущего узла
- `$from.node(i)` - получение родительского узла на уровне `i`
- Если хотя бы один родитель имеет тип `'table'` → курсор в таблице

#### 3. Условный рендер Toolbar
**Решение**:
```typescript
{isInTable ? (
...
**Альтернативы**:
- ❌ Показывать оба Toolbar одновременно - перегрузка UI
- ❌ Использовать только TableFloatingMenu (BubbleMenu) - не всегда виден, занимает место в контенте
- ✅ Полная замена Toolbar - чистый UI, только нужные инструменты

### ⚠️ Важные детали

#### События редактора
**transaction vs selectionUpdate**:
- `transaction` - любое изменение документа (ввод текста, удаление, вставка)
- `selectionUpdate` - изменение позиции курсора (клик, стрелки, Tab)

Нужны **оба события**, потому что:
- При вводе текста в ячейке → срабатывает `transaction`, но не `selectionUpdate`
- При клике мышью → срабатывает `selectionUpdate`

#### Cleanup подписок
Критично отписаться от событий в `useEffect` cleanup:
```typescript
return () => {
...
Иначе будет **memory leak** (утечка памяти) при размонтировании компонента.

#### ProseMirror Node Structure
```
doc
├── paragraph
├── table ← node.type.name === 'table'
│   ├── tableRow
│   │   ├── tableCell ← здесь курсор
│   │   ├── tableCell
│   │   └── tableCell
│   └── tableRow
│       ├── tableCell
...
Проверка `$from.node(i).type.name === 'table'` поднимается вверх по дереву и находит родителя-таблицу.

### 📈 Результаты

**До**:
- Основной Toolbar (10+ кнопок)
- TableFloatingMenu (BubbleMenu над таблицей, 7 кнопок)
- TableCellOptionsMenu (меню при клике на ячейку)
- **Проблема**: визуальная перегрузка, путаница

**После**:
- При курсоре вне таблицы: **Toolbar** (все инструменты)
- При курсоре в таблице: **TableToolbar** (только инструменты для таблицы)
- **Преимущество**: контекстная панель, экономия места, ясность

### 🎨 UX улучшения
1. **Интуитивность** - инструменты появляются автоматически
2. **Экономия места** - критично для Side Panel (~400px)
3. **Фокус** - только нужные кнопки, без отвлекающих элементов
4. **Согласованность** - тот же стиль shadcn/ui, те же размеры кнопок

### 🔧 Техническая архитектура

**Файлы**:
- `src/components/TiptapEditor/TableToolbar.tsx` - новый компонент
- `src/components/Editor/EditorArea.tsx` - модификация с `isInTable` state
- `docs/changelog.md` - обновлен
- `docs/Diary.md` - эта запись

**Зависимости**:
- React hooks: `useState`, `useEffect`
- Tiptap: `Editor`, события `transaction`, `selectionUpdate`
- shadcn/ui: `Button`, `Separator`, `DropdownMenu`
- i18n: `useTranslation()`

---

## 📅 [2025-11-02] | Интерактивное меню для ячеек таблиц

### 📊 Задача
Реализовать функционал работы с отдельными ячейками таблиц, как в Tiptap Table Node:
1. Визуальное выделение активной ячейки (фиолетовая рамка)
2. Круглая кнопка (call point) для вызова меню
3. Меню с настройками ячейки (Color, Alignment, Clear contents)

### 🎯 Вдохновение
Документация Tiptap: https://tiptap.dev/docs/ui-components/node-components/table-node
- Скриншот 1: Фиолетовая рамка вокруг активной ячейки + call point
- Скриншот 2: Меню с опциями Color, Alignment, Clear contents

### ✅ Решения

#### 1. Визуальное выделение ячейки
**Проблема**: Стандартный TableCell не имеет визуального выделения при фокусе.

**Решение**: 
- Добавлены CSS стили для селекторов `:focus-within` и `.ProseMirror-selectednode`
- Фиолетовая рамка: `outline: 2px solid rgb(147, 51, 234)` (purple-600)
- Использован `outline` вместо `border`, чтобы не влиять на размеры ячейки

#### 2. Call Point кнопка
**Проблема**: Нужно добавить кнопку внутри ячейки, но не через React NodeView (слишком сложно).

**Решение**:
- Создан `TableCellWithMenu` extension, расширяющий стандартный `TableCell`
- Добавлен ProseMirror Plugin с `handleDOMEvents.mouseover`
- При наведении на ячейку динамически создается call point кнопка
- CSS: круглая кнопка, фиолетовая, с иконкой из 4 точек (2x2 grid)
- Показывается только при hover или когда ячейка активна

**Альтернативы**:
- ❌ React NodeView - слишком сложно для простой кнопки
- ❌ Decorations - не подходит для интерактивных элементов
- ✅ DOM манипуляция через Plugin - простое и эффективное решение

#### 3. Меню с настройками
**Проблема**: Существующий `TableCellMenu` использовал `BubbleMenu`, который показывается автоматически и нельзя привязать к кнопке.

**Решение**:
- Переписан `TableCellMenu` на `Popover` из shadcn/ui
- Создан `TableCellMenuManager` для управления состоянием меню
- Обработчик кликов на call point кнопку открывает меню
- Меню позиционируется относительно call point
- Подменю для Color и Alignment реализованы как вложенные div с absolute позиционированием

**Функционал меню**:
- **Color**: 7 предустановленных цветов + default (transparent)
- **Alignment**: left, center, right (с визуальной индикацией активного)
- **Clear contents**: удаляет весь контент ячейки

#### 4. Поддержка backgroundColor attribute
**Проблема**: Стандартный TableCell не поддерживает фоновый цвет.

**Решение**:
- Добавлен атрибут `backgroundColor` в `TableCellWithMenu`
- Сохраняется в `data-background-color` HTML атрибуте
- Применяется через CSS переменную `--cell-bg-color`
- Команда `setCellAttribute('backgroundColor', color)` для изменения

#### 5. Интеграция в редактор
**Изменения**:
- `TiptapEditor.tsx`: заменен `TableCell` на `TableCellWithMenu`
- Добавлены компоненты `TableCellMenuManager` и `TableFloatingMenu`
- `TableFloatingMenu` сохранен для управления строками/столбцами

### ⚙️ Технические детали

**Архитектура**:
```
TableCellWithMenu (Extension)
├── Атрибуты: backgroundColor, verticalAlign, align (от родителя)
├── ProseMirror Plugin: добавление call point кнопок
└── Команды: setCellAttribute

TableCellMenuManager (React Component)
├── useState: isMenuOpen, anchorElement
├── useEffect: обработчик кликов на call point
└── Рендер: TableCellMenu

TableCellMenu (React Component)
├── Popover с тремя опциями
├── Color submenu: 7 цветов
├── Alignment submenu: left/center/right
└── Clear button
```

**CSS**:
- `.table-cell-handle`: круглая кнопка (20px, фиолетовая)
- `.table-cell-handle::before`: иконка (4 точки в сетке)
- `td:hover .table-cell-handle`: показывать при hover
- `td.ProseMirror-selectednode`: фиолетовая рамка
...
handleDOMEvents: {
  mouseover: (view, event) => {
    ...
    if (!cell.querySelector('.table-cell-handle')) {
      // Создаем и добавляем call point
    }
  }
}
```
...
1. **Performance**: Создание call point на каждый hover может быть медленным при большом количестве ячеек
   - **Решение**: Кнопки создаются один раз и переиспользуются
   
2. **Memory leaks**: Динамически созданные элементы могут не очищаться
   - **Мониторинг**: Проверить в DevTools после интенсивного использования

3. **Accessibility**: Call point не доступен с клавиатуры
   - **TODO**: Добавить keyboard navigation (Tab для фокуса на кнопку)

### 🎨 UX решения

- Фиолетовый цвет (purple-600) для единообразия с дизайн-системой
- Плавные переходы (transition: all 0.2s ease)
- Scale эффект при hover на call point (scale(1.1))
- Иконка из 4 точек для узнаваемости (как в мобильных приложениях)

### 📊 Результаты

✅ Визуальное выделение активной ячейки работает  
✅ Call point появляется при hover и клике на ячейку  
✅ Меню открывается при клике на call point  
✅ Все опции меню функциональны  
✅ Цвет фона ячейки сохраняется в HTML  
✅ 0 ошибок линтера  
